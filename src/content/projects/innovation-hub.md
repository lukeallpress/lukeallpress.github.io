---
title: Innovation Hub
summary: The district-built web app that hundreds of Agua Fria staff use every day — pulling intervention, attendance, coaching, and teacher tools into one place they can actually rely on.
order: 0
kind: District platform
year: 2026
status: Live
tags: ['Web app', 'District platform', 'Data', 'Flagship']
demo: 'https://innovation.aguafria.org/demo-mode/'
featured: true
---

This is the project I'm proudest of. The Innovation Hub is a web application my team built in-house and now runs for the whole district — used by hundreds of employees, every day, to do the actual work of running schools. It's the clearest proof of what I believe: districts can own the software that runs them, tailored exactly to how they work, instead of renting a patchwork of systems that never quite fit.

Rather than one more tool to log into, the Hub brings the daily essentials into a single place. Its biggest features:

- **Intervention** — track and coordinate student interventions: who needs support, what's being tried, and whether it's working, so MTSS/RTI is a shared, visible practice instead of scattered spreadsheets.
- **Attendance** — surface attendance and tardy patterns early, so staff can act before small problems become big ones.
- **Coaches Corner** — one place for athletic coaches to monitor their athletes' grades and eligibility at a glance.
- **Teacher Dash** — a personalized dashboard that pulls each teacher's most-used data and tools into one clean view.

Building it in-house means it keeps evolving with the district's real needs, stays inside our own data boundaries, and turns "can you build a tool that does X?" into a yes.

<!-- Live demo is a safe, demo-mode view of the real production app used across the district. -->
